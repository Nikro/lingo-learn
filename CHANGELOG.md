...
### Changed
- **Single canonical stage manifest (ADR 0007)** — The root-level stage files (`data/en-es/{a1-1,…,b2-3}.json`) were retired and deleted. The only stage manifest is now `data/<locale>/<stage>/themes/<stage>.json`. `loadStageData` fetches that path directly (no more two-step root→canonical fallback); the `verbs[]` arrays that only the `b1-1` and `b2-1` root files carried were migrated byte-identical into the canonical manifests so the conjugation-quiz fallback keeps working. `data/validate.js` now treats any resurrected `data/<locale>/<stage>.json` as a hard error, so the two sources of truth cannot silently diverge again. `data/README.md`, `CONTRIBUTING.md`, and `PLAN.md` updated to the canonical layout. `app.js` `?v=60`; `CACHE_VERSION` 17 → 18.
...
### Fixed
- **Conjugation-matrix crash on stub verbs** — Exercises of type `conjugation-matrix` reference verbs that, in the current data, exist only as stubs (`id`/`infinitive`/`translation`, no `conjugations` table). `startQuiz` attached the stub as `verbData`, and `submitConjugationMatrix` then threw on `verbObj.conjugations[t]` (missing map), and the matrix grid itself rendered an all-empty table. Now: quiz setup only attaches verbs that actually have a `conjugations` table, the grid renderer guards a missing map, and `submitConjugationMatrix` on a stub-verb question scores 0 with an explicit "No conjugation data available yet" message instead of throwing. Verified end-to-end in headless Chromium (a1-2 preterite-regular quiz: no JS exceptions). `app.js` `?v=60`.
- **Progress display desync after quiz** — `finishQuiz()` saved the score to localStorage via `Storage.saveProgress()`, but the stage header's `getProgress()` is a pure localStorage read with no Alpine dependency, so Alpine never re-evaluated the expression: the sidebar kept showing "Not started" until a full page reload. Added a reactive `progressVersion` counter (bumped on quiz completion, locale switch, and progress reset) that `getProgress()` now depends on, so the progress bar and "Not started" label update immediately. `app.js` `?v=58`; `CACHE_VERSION` 15 → 16.
